...
class Cart(object):
    ...
    def __init__(self, session, session_key=None):
        self._items_dict = {}    #internal use
        self.session = session
        self.session_key = session_key or self.session.session_key
            # If a cart representation was previosly stored in session, then we
    ...
    def add(self, product, price=None, quantity=1):
        ...
        if product in self._items_dict:   ### Possible Thing 
    ...
    def remove(self, pk=None):
        ...
        """
        pk=int(pk)
        if pk in self._items_dict.keys():
            del self._items_dict[pk]
            self.update_session()
    ...
    def cart_serializable(self):
        ...
        For instance:
        {
            ...
            '2': {'product_pk': 2, 'quantity': 2, price: '29.99'},
        }
        Note how the product pk serves as the dictionary key.
        """
        cart_representation = {}
